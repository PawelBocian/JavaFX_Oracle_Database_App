...
public class AdminEmployeeController extends Controller implements Initializable {
    ...
    public void addEmployeeButtononClicked(ActionEvent actionEvent) { setScene(actionEvent,Strings.ADD_EMPLOYEE_SAMPLE); }

    public void refreshEmployeeButtononClicked(ActionEvent actionEvent) { setScene(actionEvent,Strings.EMPLOYEE_SAMPLE); }

    public void deleteEmployeeButtononClicked(ActionEvent actionEvent) {

        String query = Strings.REMOVE_EMPLOYEE_BY_ID + employeesTable.getSelectionModel().getSelectedItem().getEmployeeID();

        try{
            Connection connection = DBConnection.getConnection(null);
            ResultSet rsEmployees = connection.createStatement().executeQuery(query);
            EmployeeTableObject selectedItem = employeesTable.getSelectionModel().getSelectedItem();
            employeesTable.getItems().remove(selectedItem);

        }catch(SQLException e){
            e.printStackTrace();
            JOptionPane.showMessageDialog(null,e.getMessage(),"Something went wrong! Cannot remove employees", JOptionPane.ERROR_MESSAGE);
        }
    }
